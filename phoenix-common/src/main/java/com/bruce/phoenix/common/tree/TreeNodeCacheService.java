...
import com.bruce.phoenix.common.model.constants.CacheConstant;
...
import com.bruce.phoenix.common.util.TreeUtil;
...
public abstract class TreeNodeCacheService<T extends TreeNode, F extends TreeNodeForm, V extends TreeNodeVo<V>> implements BaseService<T, F, V> {
     ...
     */
    @Override
    public abstract T queryTreeById(Long id);
     ...
     */
    @Override
    public abstract T queryTreeByCode(String code);
     ...
     */
    @Override
    public abstract List<T> queryTreeByPid(Long pId);
     ...
     */
    @Override
    @Cacheable(value = CacheConstant.TREE, key = "'tree'")
    public V tree() {
        List<T> list = getAll();
    ...
    @Override
    public T getRoot() {
        T root = queryTreeById(getParentId());
        if (root == null) {
            root = initRoot();
        }
        return root;
    ...
    public List<T> getChildNode(Long id) {
        T t = queryTreeById(id);
        return queryLikeCode(getCodePrefix(t.getCode()));
    }
    ...
    private String generateCode(Long parentId) {
        ...
        if (parentId == getParentId()) {
            List<T> list = queryTreeByPid(parentId);
            int num = list.size();
            String code = String.valueOf(getStartPrefix() + num);
            if (code.length() < getCodeSize()) {
                code = StrUtil.padAfter(code, getCodeSize(), '0');
            }
            return code;
        }
        // 找到父节点
        T parentNode = queryTreeById(parentId);
        if (parentNode != null) {
            // 获取父节点的 code
            ...
            if (StrUtil.isNotEmpty(code)) {
                ...
                List<T> list = queryTreeByPid(parentNode.getId());
                // 获取当前节点下有多少个子节点
                int num = list.size();
                // 需要排除父节点本身
